...
using ProdoctorovIntegration.Tests.Common.Mocks;
using Xunit;
...
public class ScheduleServiceTests : BaseHospitalTestWithDb
{
    ...
    private ScheduleService Sut()
    {
        ...
        var mutexManagerMock = new Mock<ISharedMutexManager>();
        mutexManagerMock.Setup(x => x.LockAsync(It.IsAny<string>(), TimeSpan.Zero))
            .ReturnsAsync(new RedisLockedMutexHandle(new MockRedlock()));

        return new ScheduleService(_clientServiceMock.Object, mutexManagerMock.Object, HospitalContext, organizationNameOptionsMock.Object);
    }
